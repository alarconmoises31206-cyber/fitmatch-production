...
const FirstMessageComposer: React.FC<FirstMessageComposerProps> = ({
  ...
  const generateScaffold: any= () => {
    // Access compatibilityScore instead of score
    const compatibilityScore: any= match.compatibilityScore || 95;
    const tier: any= match.tier === 'full-fitmatch' ? 'Full FitMatch' : 'Free Spin',
    
    // Use breakdown data to generate personalized message
  const bestMatchAspect: any= Object.entries(match.breakdown || {})
    ...
    const trainerFirstName: any= trainerName.split(' ')[0];
