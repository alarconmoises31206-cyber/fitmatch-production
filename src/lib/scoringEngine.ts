export interface FitMatchScores {
  goalsAlignment: number;
  trainingStyleAlignment: number;
...
export class ScoringEngine {
  ...
  calculateGoalsAlignment(client: any, trainer: any): number {
  ...
  calculateTrainingStyleAlignment(client: any, trainer: any): number {
  ...
  calculateMotivationAlignment(client: any, trainer: any): number {
  ...
  calculateExperienceFit(client: any, trainer: any): number {
  ...
  calculateLogisticsFit(client: any, trainer: any): number {
    // Edge cases
  ...
  calculateGlobalScore(subScores: Omit<FitMatchScores, 'globalScore'>): number {
    return Math.round(
  ...
  // Helper methods
  private calculateGoalCompatibility(clientGoal: string, trainerFocus: string): number {
  ...
  private calculateToneCompatibility(clientTone: string, trainerTone: string): number {
  ...
  private calculateLocationCompatibility(clientLoc: string, trainerLoc: string): number {
    if (clientLoc === trainerLoc) return 100;
    if (clientLoc === 'virtual' || trainerLoc === 'virtual') return 80;
    return 0;
  }
  
  async generateFitMatchScore(clientData: any, trainerData: any): Promise<FitMatchScores> {
    const subScores = {
